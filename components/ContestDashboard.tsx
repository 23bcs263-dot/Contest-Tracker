...
export function ContestDashboard({ contests, username }: { contests: Contest[]; username: string }) {
  ...
  const [usernameQuery, setUsernameQuery] = useState(username);
  ...
  const loadUsername = async () => {
    ...
    if (!nextUsername || isLoadingUser) return;
    setIsLoadingUser(true);
    setUserError("");
    try {
      const response = await fetch(`/api/contests?username=${encodeURIComponent(nextUsername)}&page=1`, { cache: "no-store" });
  ...
  const loadPage = async (page: number) => {
    ...
    setUserError("");
    try {
      const response = await fetch(`/api/contests?username=${encodeURIComponent(activeUsername)}&page=${page}`, { cache: "no-store" });
  ...
  return <div className="min-h-screen bg-[#0b0f14] text-slate-100">
    <Navbar searchQuery={searchQuery} onSearchChange={(value) => { setSearchQuery(value); resetPage(); }} username={usernameQuery} onUsernameChange={setUsernameQuery} onUsernameSubmit={loadUsername} isLoading={isLoadingUser} />
    <main className="mx-auto max-w-7xl px-5 pb-16 pt-10 sm:px-8 lg:px-10">
      <section className="mb-9 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="font-mono text-xs font-medium uppercase tracking-[0.2em] text-cyan-400">{"// progress dashboard"}</p><h1 className="mt-3 text-3xl font-bold tracking-tight text-white sm:text-4xl">Your contest history</h1><p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">{activeUsername ? <>Tracking LeetCode progress for <span className="font-mono text-slate-300">@{activeUsername}</span>.</> : "Enter a LeetCode username above to load contest history."}</p></div><p className="font-mono text-xs text-slate-600">{totalContests} contests tracked</p></section>
